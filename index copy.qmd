...
```{python} 
#| content: valuebox
#| title: "Tton de CO₂anual máximo por país "
#| color: warning

co2_max = co2_percountry[co2_percountry.Year == co2_percountry.Year.max()]
co2_max = co2_max["Annual CO₂ emissions"].max()/1000_000_000

dict(value = (co2_max).round(2))
...
```{python} 
co2_max = co2_percountry[co2_percountry.Year == co2_percountry.Year.max()]
co2_max_entidad = co2_percountry[co2_max["Annual CO₂ emissions"] == co2_max["Annual CO₂ emissions"].max()]
co2_max = co2_max["Annual CO₂ emissions"].max()/1000_000_000
```
`{python} co2_max.round(2)`  Tton de CO2 emitidos por `{python} co2_max_entidad`
:::
...
```{python}
print("Promedio")
co2_mean = co2_percountry[co2_percountry.Year == co2_percountry.Year.max()]
print(f"{co2_mean["Annual CO₂ emissions"].mean()/1000_000_000:.3f} Tton CO₂")
```
::: -->
<!-- 
::: {.card}
```{python}
print("México")
co2_mex = co2_percountry[(co2_percountry.Year == co2_percountry.Year.max()) & (co2.Entity=="Mexico")]
print(f"{co2_mex["Annual CO₂ emissions"].values[0]/1000_000_000} Tton CO₂")
```
::: -->
...
```{python}
print("Mínimo != 0")
co2_min = co2_percountry[(co2_percountry.Year == co2_percountry.Year.max()) & (co2["Annual CO₂ emissions"]>0)]
print(f"{co2_min["Annual CO₂ emissions"].min()/1000:.3f} Kton CO₂")
```
:::
